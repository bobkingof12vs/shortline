...
function ObjToJson($file){
	...
	foreach($face as $fpart){
		...
		$n = calcNormal($tempn);
		if(($n[0] <= 0 and $n[1] <= 0 and $n[2] == 0) or $n[2] != 0){
			$flf[] = array(min($tempx),min($tempy),min($tempz),max($tempx),max($tempy),max($tempz),$fpart,$n);
		}
		if(($n[0] <= 0 and $n[1] >= 0 and $n[2] == 0) or $n[2] != 0){
			$frf[] = array(min($tempx),min($tempy),min($tempz),max($tempx),max($tempy),max($tempz),$fpart,$n);
		}
		if(($n[0] >= 0 and $n[1] <= 0 and $n[2] == 0) or $n[2] != 0){
			$flb[] = array(min($tempx),min($tempy),min($tempz),max($tempx),max($tempy),max($tempz),$fpart,$n);
		}
		if(($n[0] >= 0 and $n[1] >= 0 and $n[2] == 0) or $n[2] != 0){
			$frb[] = array(min($tempx),min($tempy),min($tempz),max($tempx),max($tempy),max($tempz),$fpart,$n);
		}
